...
public class LikePostCommandTests : IClassFixture<InjectionFixture>
{
    ...
    public LikePostCommandTests(InjectionFixture fixture)
    {
    ...
    [Fact]
    public async Task LikePostCommand_VerifyPostLikedSuccess()
    {
        ...
        var likePostCommandHandler = new LikePostCommandHandler(
            _postLikeRepository, _authenticatedCurrentUserInfoProvider, _userProfileRepository);

        await likePostCommandHandler.Handle(new LikePostCommand(new() { PostId = existingUserPost.Id }), CancellationToken.None);
